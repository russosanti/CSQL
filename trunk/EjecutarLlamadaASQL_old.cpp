		...
		if (found!=string::npos){	
			...
			if (encontrado!=string::npos){	
				if (ObtenerString(SQLSentencia,found+6,fin_string,NombreTabla) == 0){
					...
					printf("construcción CREATE incorrecta (sin TABLE )");
				}
			}else{
				...
				if (encontrado!=string::npos){	
					if (ObtenerString(SQLSentencia,found+5,fin_string,NombreTabla) == 0){
						...
						printf("construcción CREATE incorrecta (sin VIEW)");
					}
				}else{
					printf("construcción CREATE incorrecta (sin TABLE ni VIEW)");
				};
			};
		};
	};
...
int VerificaTipoDatoValido(string &TipoDatoCampo)
{
	if (
		...
		return 1; // Indica que no un tipo de dato válido
