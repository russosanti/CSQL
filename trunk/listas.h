...
void crearnodo(nodos **nue,Tdato_IString x)
{
...
void linfo (tlista_s *l,Tdato_IString *x)
{
	*x=(l->actual->info);
}

/* Modifica la información del item en la posición solicitada */
void lmodificar (tlista_s *l,Tdato_IString aux)
{
	l->actual->info=aux;
...
void linsertarppio(tlista_s *l, Tdato_IString x)
{
	nodos *nue;
	crearnodo(&nue,x);
	if(l->cab==NULL){
...
void linsertarfin(tlista_s *l, Tdato_IString x)
{
	nodos *nue,*aux;
	crearnodo(&nue,x);
	if (l->cab==NULL){
...
void linsertarorden(tlista_s *l, Tdato_IString x, char torden){
nodos *nue;
nodos *aux;
...
if(l->cab == NULL){
	...
	if((torden=='a' && stricmp(l->cab->info.clave,x.clave)>0) || (torden=='d' && stricmp(l->cab->info.clave,x.clave)<0)){
		aux=l->cab->ant;
		nue->sig=l->cab;
		nue->ant=aux;
		aux->sig=nue;
		...
		aux=l->cab;
		while((aux->sig!=l->cab) && ((torden=='a' && stricmp(aux->sig->info.clave,x.clave)<0) || (torden=='d' && stricmp(aux->sig->info.clave,x.clave)>0))){
			aux=aux->sig;
		 }
		 nue->sig=aux->sig;
		 nue->ant=aux;
...
if(l->cab!=NULL){
	aux=l->cab;
	if(l->cab->sig==l->cab){
		if(stricmp(l->cab->info.clave,clave)==0){
			*existe=1;
		}
	}else{
		if(stricmp(l->cab->info.clave,l->cab->sig->info.clave)<0){
			while((aux->sig!=l->cab) && (stricmp(clave,aux->info.clave)>0)){
				aux=aux->sig;
			}
		}else{
			while((aux->sig!=l->cab) && (stricmp(clave,aux->info.clave)<0)){
		...
		*existe=(stricmp(clave,aux->info.clave)==0);
	}
}
if(*existe){
	l->actual=aux;
